...
3. **確認ダイアログで実行を確定**
   - ⚠️ オーナーはグループ全体を削除できます
   - ⚠️ メンバーは自分だけグループから退出できます
   - ⚠️ 削除されたデータは復元できません
...
| プラン | グループ数 | 1グループのメンバー数 | グループ一覧のバナー広告 |
| ------ | ---------- | ---------------------- | ------------------------ |
| Free | 最大3個 | 最大10人 | 表示される |
| Premium | 上限なし | 上限なし | 表示されない |

PremiumからFreeへ切り替えるには、グループ数を3個以下にし、すべてのグループのメンバー数を10人以下にする必要があります。上限を超えるグループがある場合は、削除またはメンバー数の調整後に切り替えてください。
...
### Q: Freeプランではグループをいくつ作れますか？

**A:** 最大3個です。Premiumプランでは上限はありません。

### Q: PremiumからFreeに切り替えられません。

**A:** グループ数を3個以下にし、すべてのグループのメンバー数を10人以下にしてください。

### Q: データは安全ですか？

**A:** はい。共有グループのアイテム名と鍵交換データは、グループ鍵を使って暗号化したうえでFirebaseクラウドに保存されます。なお、アプリ全体のすべてのデータが一律に暗号化されているわけではなく、認証情報や設定などの一部は既存の基盤で管理されています。

### Q: 他の端末でも使用できますか？

**A:** はい。現在は Android / iOS / Windows で利用できます。使用中の端末間でもFirestore経由で同期されます。

### Q: アカウントを削除したい場合は？

**A:** 設定画面からアカウント削除を実行できます。削除前に再認証が必要になる場合があります。
...
_このガイドは Go Shop v1.0.0 対応版です。_
_最新の情報については、アプリ内の「アプリについて」も併せてご確認ください。_
