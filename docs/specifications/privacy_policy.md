...
| 買い物リスト   | リスト名、各アイテムの購入状態                |
| 共有アイテム   | `name` はグループ鍵で暗号化済み、復号後に表示 |
| 鍵交換データ   | `encryptedGroupKey` などの暗号鍵情報          |
| ホワイトボード | 描画ストロークデータ                          |
...
- **グループデータ（グループ名・リスト名など）**: そのグループに所属するメンバー（`allowedUid` に登録されたユーザー）のみ読み書きが可能です。グループに参加していないユーザーは一切アクセスできません。
- **共有アイテム名**: グループ鍵を用いて暗号化された値として保存され、必要時のみ復号して表示します。平文の `name` が保存されることは原則ありません。
- **鍵交換データ**: `encryptedGroupKey` などの鍵配布情報は、各メンバー固有の秘密情報で復号可能な形式で保存されます。
...
- List information (list names, item information) ※ Includes both Shopping List mode and TODO List mode
...
- Crash reports (Android/iOS): Firebase Crashlytics
- Crash reports (Windows): Sentry
...
- Purpose: Crash reporting and error monitoring (primarily for Windows)
- Information collected: Stack traces at crash time, device information, OS version, app version
