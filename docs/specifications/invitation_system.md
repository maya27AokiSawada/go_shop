...
**コレクション:** `SharedGroups/{groupId}/invitations/{invitationId}`
...
```javascript
{
  ...
  inviterDisplayName: "Maya",
    ...
    // グループ情報
    sharedGroupId: "1762...",
    groupId: "1762...",  // sharedGroupId のエイリアス
  groupName: "家族の買い物",
  groupOwnerUid: "VqNEozvTyXXw55Q46mNiGNMNngw2",
  ...
  invitationToken: "eyJncm91cElkIjoi...",
    ...
    // 使用回数・ステータス管理
    status: "pending",   // pending | accepted | used | expired
    maxUses: 5,
    currentUses: 0,
    usedBy: [],
    lastUsedAt: null,
...
- QRに格納されるペイロードは v3.1 の軽量形式がデフォルト（`invitationId`, `sharedGroupId`, `securityKey`, `type`, `version`）。
- v3.1はスキャン時に Firestore から詳細を再取得する。
...
│   - metadata.acceptorUid             │
│   - metadata.invitationId            │
└─────────────────────────────────────┘
    ↓
招待元（オーナー）端末の通知リスナーが受信
    ↓
┌─────────────────────────────────────┐
│ 招待元がグループ更新                 │
│                                      │
...
│ invitations/{invitationId}:          │
│   - currentUses += 1                 │
│   - usedBy に acceptorUid を追加      │
│   - status: accepted or used         │
└─────────────────────────────────────┘
    ↓
受諾者/既存メンバーへ確認通知
    ↓
Firestore → Hive 同期・UI更新
```
...
| `accepted` | 受諾済み（上限未満） | ✅ YES   | 「この招待は使用中です（残り回数あり）」   |
| `used`     | 上限到達             | ❌ NO    | 「この招待は使用回数の上限に達しています」 |
...
| `lib/pages/shared_group_page.dart`              | QR招待ボタンとダイアログ   |
...
**Q3: 何人まで招待できますか？**
A: 1つの招待ドキュメントあたり既定5人までです（`maxUses`）。将来的に設定可能化を検討しています。
...
- **パートナー招待**: 将来拡張の予約値（現行実装は個別招待運用が中心）
