...
**Purpose**: アイテム追加後の成功スナックバーをなくし、連続追加時の操作テンポを優先する。

**Background**: 実機確認と UX 観点の再検討の結果、アイテム追加は直後に一覧へ反映されるため、成功メッセージは見れば分かる状態だった。追加のたびにスナックバーが出ると、反復操作のテンポだけが悪化する。

**Solution**:

新規アイテム追加時の成功スナックバーを削除し、失敗時のみエラーを表示する形に変更した。更新成功時のスナックバーは従来どおり残している。

```dart
// ✅ After: 新規追加は UI 反映のみ、成功スナックバーなし
...
- `lib/widgets/shared_item_edit_modal.dart` - 新規アイテム追加成功時のスナックバーを削除
...
### アイテム追加成功スナックバーが反復操作のテンポを阻害していた ✅

- **症状**: アイテム追加成功時のスナックバー表示が、連続追加時の操作テンポを悪化させていた
- **原因**: 追加結果が一覧で即座に見えるにもかかわらず、成功通知を毎回表示していた
- **対処**: 新規アイテム追加成功時のスナックバーを廃止し、失敗時のみ通知する形に変更
- **状態**: 修正完了
...
4. ✅ 新規アイテム追加成功時のスナックバーを廃止し、反復追加UXを改善（完了日: 2026-03-10）
