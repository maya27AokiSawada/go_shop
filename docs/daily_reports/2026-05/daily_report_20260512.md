...
- [x] Firestoreインデックスのデプロイ（新プロジェクト your-prod-firebase-project-id）
- [x] アカウント削除機能のバグ修正
...
Firebase プロジェクト移行（`legacy-prod-firebase-project-id` → `your-prod-firebase-project-id`）後に新規アカウントを作成すると、アプリがマルチモードで起動するという不具合が報告された。
...
### 2. Firestore 複合インデックスのデプロイ（your-prod-firebase-project-id） ✅

**Purpose**: 通知リスナーの `failed-precondition` エラーを解消

**Background**:

Firebase プロジェクト移行後、新プロジェクト `your-prod-firebase-project-id` に `firestore.indexes.json` で定義済みのインデックスがデプロイされていなかった。
...
```bash
firebase deploy --only firestore:indexes --project your-prod-firebase-project-id
```
...
- **対処**: `firebase deploy --only firestore:indexes --project your-prod-firebase-project-id` 実行
- **状態**: 修正完了 ✅
...
1. ✅ Firebase プロジェクト移行（legacy-prod-firebase-project-id → your-prod-firebase-project-id）
...
5. ✅ Firestoreインデックスデプロイ（your-prod-firebase-project-id）
...
- 🔲 上記 ⏳ 項目の実機動作確認・最終バグ修正
- 🔲 新プロジェクト `your-prod-firebase-project-id` での総合動作確認（グループ作成・招待・通知フロー）
