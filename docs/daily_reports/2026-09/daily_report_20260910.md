...
- [x] ビルド番号を上げて署名済みリリース AAB をビルドし、クローズドテストへ回す
- [x] 実機 2 台（SH-54D / Pixel 9）に build 35 を導入してテストを実施する
- [~] グループメンバー暗号化の実機復号バグ（9/9 中断分）を特定し修正する（原因特定・A+B 実装完了。実機での修正版 E2E 確認は次回）
...
1. **修正版（build 36）を実機 2 台で E2E 確認**: 再インストール直後のメンバー端末で、
   リストを一度も開かずにメンバー管理画面を開いて `name` / `contact` が平文で出ること。
   オーナー端末（`iwk***`）でも確認
