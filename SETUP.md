...
- `android/app/src/prod/google-services.json`（`--flavor prod` で使用）
- `android/app/src/dev/google-services.json`（`--flavor dev` で使用）

補助ファイル:

- `android/app/google-services.json` は共通ビルド向けの補助として置いてもよいが、
  flavor 実行時は `src/<flavor>/google-services.json` が優先される
...
4. デスクトップで障害監視したい場合は Sentry プロジェクトを作成する
...
## 10. ビルド前の Firebase プレースホルダーチェック

`YOUR_*` などのダミー値が残っていると、実機で `API key not valid` が発生します。
以下のスクリプトでビルド前にチェックできます。
...
`BUILD_ANDROID_APP.bat` では `prod` チェックを自動実行するようにしています。
...
- `android/app/src/prod/google-services.json` と `android/app/src/dev/google-services.json` に配置されているか
