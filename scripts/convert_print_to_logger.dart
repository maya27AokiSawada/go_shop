...
ConversionResult convertPrintToLogger(String content, String filePath) {
  ...
  // 正規表現でprint文を検索して置換
  final printPattern = RegExp(r'print\((.+?)\);', multiLine: true, dotAll: true);
  ...
  if (matches.isNotEmpty) {
    ...
    for (int i = matches.length - 1; i >= 0; i--) {
      ...
      // 絵文字に基づいてログレベルを決定
      if (argument != null) {
        if (argument.contains('❌') || argument.contains('Failed') || argument.contains('Error') || argument.contains('エラー')) {
          loggerMethod = '_logger.e';
        } else if (argument.contains('⚠️') || argument.contains('警告') || argument.contains('Warning')) {
  ...
  if (changed && !updatedContent.contains("import 'package:logger/logger.dart'")) {
