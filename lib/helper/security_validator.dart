...
class SecurityValidator {
  ...
  static bool validateMemberIdConsistency(PurchaseGroup group, String currentUid) {
    // 開発環境ではスキップ
  ...
  static bool validateMemberAccess(PurchaseGroup group, String currentUid) {
    ...
    return group.members?.any((member) => 
      ...
      member.isInvitationAccepted == true
    ) ?? false;
  }
    ...
    switch (operation) {
      case 'read':
        if (!validateMemberAccess(group, currentUid)) {
          throw SecurityException('Read access denied: User is not a group member');
        }
        break;
      case 'write':
      case 'delete':
        if (!validateOwnerAccess(group, currentUid)) {
          throw SecurityException('Write access denied: User is not the group owner');
        }
        break;
      case 'create':
