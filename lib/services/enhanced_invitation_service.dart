...
class EnhancedInvitationService {
  ...
  Future<List<GroupInvitationOption>> findInvitableGroups(String targetEmail) async {
    try {
      ...
      final userGroups = await FirestoreCollections.getUserPurchaseGroups(currentUser.uid).get();
      final invitableGroups = <GroupInvitationOption>[];
      
      for (final doc in userGroups.docs) {
        final firestoreGroup = FirestorePurchaseGroup.fromFirestoreData(doc.data());
        ...
        if (firestoreGroup.canInviteUsers(currentUser.uid)) {
          ...
          final existingMember = firestoreGroup.members?.firstWhere(
            ...
            orElse: () => const PurchaseGroupMember(memberId: '', name: '', contact: '', role: PurchaseGroupRole.member),
          );
  ...
  Future<void> _sendSingleInvitation({
    ...
    // Get group document
    final groupDoc = await FirestoreCollections.getPurchaseGroupDoc(ownerUid, groupId).get();
    if (!groupDoc.exists) {
      throw Exception('グループが見つかりません');
    }
    
    final firestoreGroup = FirestorePurchaseGroup.fromFirestoreData(groupDoc.data()!);
  ...
  Future<void> acceptInvitation({
    ...
    try {
      ...
      developer.log('✅ 招待受諾完了: UID $userUid → グループ「${firestoreGroup.groupName}」');
    } catch (e) {
  ...
  Future<List<PendingInvitation>> findPendingInvitations(String userEmail) async {
    try {
...
final enhancedInvitationServiceProvider = Provider<EnhancedInvitationService>((ref) {
