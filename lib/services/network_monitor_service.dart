...
class NetworkMonitorService {
  ...
  /// Firestore接続をチェック
  ///
  /// Firestoreの`SharedGroups`コレクションに対してサーバーからの
  /// データ取得を試み、5秒以内に成功すればオンラインと判定する。
  ///
  ...
  Future<bool> checkFirestoreConnection() async {
    ...
    try {
      ...
      // 接続成功
      AppLogger.info('✅ [NETWORK_MONITOR] Firestore接続成功');
      ...
      // タイムアウト
      AppLogger.warning('⏱️ [NETWORK_MONITOR] Firestore接続タイムアウト: $e');
  ...
  void _updateStatus(NetworkStatus status) {
    if (_currentStatus != status) {
      ...
      AppLogger.info('📡 [NETWORK_MONITOR] 状態変更: $status');
    }
  }
