...
- このリポジトリには実運用の Firebase Project ID や API Key は含めません。
- `Flavor.dev` と `Flavor.prod` それぞれに対して、自分の Firebase プロジェクトを割り当ててください。
- Firebase の切り替えは `lib/firebase_options.dart` と `lib/flavors.dart` の構成に従います。
- 実際の設定手順は [SETUP.md](SETUP.md) を参照してください。
...
- Firebase / AdMob / Sentry の実値は、追跡除外された `.env` やネイティブ設定ファイルにのみ配置します
- 追跡対象ファイルには設定方法とキー名だけを残し、実際の値は残さない方針です
...
- 招待受諾後の keyExchangeEvents 作成漏れを修正し、メンバー側が鍵を復号できる経路を復旧
- Firestore Security Rules の不足を補い、`keyExchangeEvents` の read/create/update/delete を整備
- グループ鍵の生成・ローテーションをオーナー限定に制約し、誤発火を防止
- 既存メンバーへの再配布における再暗号化状態の判定とブロック条件を明確化
- メンバー追加直後の鍵解決を再試行付きで安定化
- QR 招待の読取タイミング差に対する再試行と後方互換対応を整備
- 共有アイテム名の暗号化と再暗号化フローを強化して、平文露出を防止
