...
- **グループ離脱**（メンバーのみ）
  - オーナーへ離脱通知を送る処理を非ブロッキング（ノンブロッキング）で実行する。
  - 自分自身のメンバー情報をプールから除外し、`removeMember` 等を呼び出して非同期的に Firestore と Hive から自分を削除する。
  - **重要（UX改善/他者オーナーオフライン対策）**: ネットワーク切断や他オーナーのオフライン状態に影響されることなく即座に離脱を完了させるため、ローカルの Hive から当該グループと紐づくすべての買い物リスト（SharedList）を即座に削除（論理・物理削除）し、さらに `allGroupsProvider` を `ref.invalidate` して UI を即座にリアクティブ更新・反映する。
  - オーナーは離脱不可（削除のみ）
